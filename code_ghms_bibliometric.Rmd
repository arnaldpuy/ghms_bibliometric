---
title: "The treatment of uncertainties in global water models"
subtitle: "R code"
author: "Arnald Puy"
header-includes:
...
loadPackages(c(
  ...
  "qdapRegex", "tidytext", "igraph", "ggraph", "benchmarkme", "parallel"))
...
```{r bibliometric_analysis, dependson=c("models_vec", "fun_clean_text"), results="hide"}
...
# Keywords analysis: "uncertainty" and "sensitivity"

```{r keywords, dependson=c("models_vec", "fun_clean_text"), results="hide", message=FALSE, warning=FALSE}
...
```{r arrange_data, dependson=c("keywords", "bibliometric_analysis")}
...
```{r descriptive, dependson="arrange_data"}
...
```{r cumulative_plot, dependson="bibliometric_analysis", fig.height=2.5, fig.width=2.5, warning = FALSE}
...
```{r fraction_unc, dependson="arrange_data", fig.height=2.5, fig.width=2.5}

# FRACTION OF STUDIES WITH UNCERTAINTI AND SENSIT* AS KEYWORDS #################

full.dt <- full.dt[, `:=` (uncertainti = str_detect(keywords, keywords_vec_stemmed[1]), 
                           sensit = str_detect(keywords, keywords_vec_stemmed[2]))] 
...
```{r keywords_time, fig.height=2.5, fig.width=3.5, dependson=c("arrange_data", "fraction_unc")}
...
```{r plot_dist, dependson="arrange_data", warning=FALSE, fig.width=3, fig.height=2}

# PLOT HISTOGRAM ###############################################################

dist.plot <- full.keyword.dt[, .N, .(name, keyword)] %>%
  .[, keyword:= factor(keyword, levels = keywords_vec_stemmed)] %>%
  ggplot(., aes(N, fill = keyword)) +
...
# How many articles have 1, 2, ... N mentions of uncertainti and sensit in the 
# main text ------------------------------------------------------
full.keyword.dt[, .N, .(name, keyword)] %>%
  .[, .(number.articles = .N), .(N, keyword)] %>%
  .[.[order(N),.I[1:.N],"keyword"]$V1]
```

```{r merge_plots_descriptive, fig.height=4.8, fig.width=5.7, dependson=c("keywords_time", "fraction_unc", "cumulative_plot", "plot_dist"), warning=FALSE}
...
```{r plot_journals, dependson="bibliometric_analysis", warning=FALSE}
...
```{r word_clouds, dependson="arrange_data", device="pdf", fig.height=3, fig.width=3.5}
...
```{r tokens, dependson=c("models_vec", "keywords")}
...
```{r plot_tokens, dependson="tokens", fig.height=4.5, fig.width=5}
...
```{r plot_permodel, dependson="tokens", fig.height=6, fig.width=5.5}
...
```{r plot_networks, dependson="tokens", fig.height=4.5, fig.width=4.5}
